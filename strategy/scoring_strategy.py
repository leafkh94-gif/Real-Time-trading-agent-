...
Detectors are documented heuristics. They encode the structure faithfully but
WILL need calibration on real data — none of this is a proven edge.
"""
...
class MarketData:
    epic: str
    m15: pd.DataFrame      # columns: open, high, low, close, volume (closed candles, oldest→newest)
    daily: pd.DataFrame    # same columns, daily timeframe
    now_utc: dt.datetime
...
# ── Pattern detectors ─────────────────────────────────────────────────────────
# Each returns a dict or None:
#   {direction, pattern, bonus(0..max), broken_level, ref_low, ref_high, confirm_price}

def _detect_sweep_bos(df: pd.DataFrame, a: pd.Series) -> Optional[dict]:
    """Liquidity sweep of a swing extreme, reclaim, then BOS through the
    intervening swing in the opposite direction. Breakout-type entry."""
    ...
    # Bullish: sweep a recent swing low, reclaim, then close above the swing high after it
    ...
    if recent_lows and recent_highs:
        li, lp = recent_lows[-1]
        # an intervening swing high that comes after the swing low
        later_highs = [(i, p) for i, p in recent_highs if i > li]
        if later_highs:
            hi, hp = later_highs[-1]
            swept = any(low[j] < lp and close[j] > lp for j in range(li, min(li + 8, n)))
            bos = close[last] > hp and close[last - 1] <= hp  # fresh break this candle
            if swept and bos:
...
def _detect_flag(df: pd.DataFrame, a: pd.Series) -> Optional[dict]:
    ...
    if n < 25:
        return None
    close = df["close"]; atr_now = float(a.iloc[-1])
    imp_window = close.iloc[-15:-5]
    impulse = imp_window.iloc[-1] - imp_window.iloc[0]
    if abs(impulse) < 3 * atr_now:
        return None
    consolidation = df.iloc[-5:]
    cons_range = consolidation["high"].max() - consolidation["low"].min()
    if cons_range > 1.5 * atr_now:                 # not a tight flag
        return None
...
def _detect_sd_rejection(df: pd.DataFrame, a: pd.Series) -> Optional[dict]:
    """Price reaches a prior swing level and prints a rejection wick. Rejection entry."""
    highs, lows = ind.swings(df)
    n = len(df); last = n - 1
    ...
    # Demand bounce: long lower wick at/below a prior swing low, close back up
    ...
    # Supply rejection: long upper wick at/above a prior swing high, close back down
...
def _detect_reversal(df: pd.DataFrame, a: pd.Series) -> Optional[dict]:
    """Double bottom/top (and simplified H&S as a triple variant). Rejection entry."""
    highs, lows = ind.swings(df)
    n = len(df); last = n - 1
    atr_now = float(a.iloc[-1]); c = float(df["close"].iloc[-1]); prev = float(df["close"].iloc[-2])

    # Double bottom: two comparable swing lows, neckline = swing high between, close above it
...
def _detect_news_retest(df: pd.DataFrame, a: pd.Series) -> Optional[dict]:
    ...
    Rejection entry. (Kept simple; complements the news-blackout logic.)"""
    n = len(df)
    if n < 20:
        return None
    atr_now = float(a.iloc[-1])
    ...
    if abs(move) < 2.5 * atr_now:
        return None
    level = float(window["close"].iloc[0])           # approx broken level
    c = float(df["close"].iloc[-1]); l = float(df["low"].iloc[-1]); h = float(df["high"].iloc[-1])
...
def _technical_confirmation(df: pd.DataFrame, direction: str) -> tuple[int, list[str]]:
    ...
    ema50 = float(ind.ema(close, 50).iloc[-1]); price = float(close.iloc[-1])
    want_buy = direction == "buy"
    agree = []
    if (r > 50) == want_buy:
        agree.append(f"RSI {r:.0f}")
    if (macd_h > 0) == want_buy:
        agree.append("MACD")
    if (price > ema50) == want_buy:
        agree.append("EMA50")
    k = len(agree)
...
def _daily_bias(daily: pd.DataFrame, direction: str) -> tuple[int, str]:
    ...
    if spread < 0.001:                               # intertwined -> neutral
        return C.BIAS_NEUTRAL, "neutral"
    up = e50 > e200 and price > e200
    down = e50 < e200 and price < e200
    if up and direction == "buy":
        return C.BIAS_ALIGNED, "aligned-up"
    if down and direction == "sell":
        return C.BIAS_ALIGNED, "aligned-down"
...
def _choppy(df: pd.DataFrame, a: pd.Series) -> bool:
    """Proxy for a directionless market: EMA50 slope tiny relative to ATR."""
...
def _build_levels(epic: str, det: dict, atr_now: float) -> Optional[dict]:
    ...
    if ptype == "breakout":
        ...
        half = lvl + 0.5 * (extreme - lvl) if direction == "buy" else lvl - 0.5 * (lvl - extreme)
        # if 50% retrace is farther than 1.5*ATR from the level, deepen toward the level
        if abs(half - lvl) > 1.5 * atr_now:
            entry = lvl + np.sign(extreme - lvl) * 0.3 * atr_now
        else:
            entry = half
    else:  # rejection -> confirmation-candle close
        entry = det["confirm_price"]

    # structural stop beyond the reference extreme, clamped to ATR band
    if direction == "buy":
    ...
    tp2 = entry + dist * (C.MIN_RR + 1.0) if direction == "buy" else entry - dist * (C.MIN_RR + 1.0)
...
class ScoringStrategy:
    ...
    def evaluate(self, md: MarketData) -> Optional[Signal]:
        ...
        # Factor 2 — technical confirmation
        ...
        if ind.round_number_near(price, self.cfg["round_step"], self.cfg["round_prox"]):
            add += C.ROUND_NUMBER_BONUS; reasons.append("Near round number (+5)")
        vol = df["volume"]
        if len(vol) >= 2 and float(vol.iloc[-1]) > float(vol.iloc[-2]):
            add += C.VOLUME_CONFIRM_BONUS; comp["volume_confirm"] = C.VOLUME_CONFIRM_BONUS
        if _high_atr(a):
            add += C.HIGH_ATR_PENALTY; reasons.append("High volatility (-10)")
        if _choppy(df, a):
            add += C.CHOPPY_PENALTY; reasons.append("Choppy market (-10)")
        comp["additional"] = add
