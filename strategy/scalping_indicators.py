"""
Lightweight indicator helpers for Plan B.

Pure pandas/numpy implementations — no TA-Lib dependency, so this drops
straight into strategy/indicators.py alongside your existing indicator
functions (or as a standalone module imported by scalping_strategy.py).

All functions expect a pandas DataFrame with columns:
    open, high, low, close, volume
indexed in chronological order (oldest -> newest).
"""
...
def ema_slope_pct(series: pd.Series, lookback: int = 3) -> float:
    ...
    expressed relative to the value `lookback` bars ago.
    """
...
def swing_high(df: pd.DataFrame, lookback: int, confirm_bars: int) -> float | None:
    """
    Most recent confirmed swing high within `lookback` bars:
    a high that is the max of a window of `confirm_bars` on each side.
    Returns None if no confirmed pivot is found.
    """
    highs = df["high"]
...
def swing_low(df: pd.DataFrame, lookback: int, confirm_bars: int) -> float | None:
    """Most recent confirmed swing low within `lookback` bars."""
    lows = df["low"]
