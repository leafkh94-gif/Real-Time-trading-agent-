...
zoneinfo, so they stay correct across the EDT/EST switch (the original UTC-hardcoded
windows silently broke in winter — this fixes that).
"""
...
# ── News blackout (Section V) — anchored to Eastern release times ──────────────
# Brackets the two main US data slots: 08:30 ET and 10:00 ET.
_NEWS_BLACKOUTS_ET = [
...
def in_news_blackout(now_utc: dt.datetime) -> bool:
    et = now_utc.replace(tzinfo=dt.timezone.utc).astimezone(_ET)
    # US data releases are weekday-only
    if et.weekday() >= 5:
        return False
